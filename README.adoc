...
# Yet Another Picoprobe (Fork)

## Quick Start (this fork)

This fork replaces the original Makefile-based build with **CMake Presets** managed by the
https://marketplace.visualstudio.com/items?itemName=raspberry-pi.raspberry-pi-pico[Raspberry Pi Pico VS Code extension].

### Prerequisites

1. Install the **Raspberry Pi Pico** extension in VS Code / Cursor
2. The extension automatically installs the SDK, toolchain, and tools under `~/.pico-sdk/`
3. Clone with submodules:
+
[source,bash]
----
git clone git@github.com:david-pedrosa/yapicoprobe.git
cd yapicoprobe
...
**From VS Code:** Use the Pico extension's compile button. Output goes to `build/`.

**From CLI (single board):**
[source,bash]
----
GIT_HASH=$(git rev-parse --short HEAD)
cmake --preset pico -DGIT_HASH=$GIT_HASH
cmake --build --preset pico-build
# Output: build/picoprobe.uf2
----

**From CLI (all boards in parallel):**
[source,bash]
----
GIT_HASH=$(git rev-parse --short HEAD)
for board in pico-debug pico_w-debug pico_debug_probe-debug pico2-debug; do
  cmake --preset $board -DGIT_HASH=$GIT_HASH
  cmake --build --preset ${board}-build &
done
wait
# Output: _build/<board>/picoprobe.uf2
----
...
[%header]
|===
| Preset | Board | Build Type | Output Dir

| `pico` | Pico (RP2040) | Release | `build/`
| `pico_w` | Pico W (RP2040 + WiFi) | Release | `build/`
| `pico_debug_probe` | Pico Debug Probe | Release | `build/`
| `pico2` | Pico 2 (RP2350) | Release | `build/`
| `pico-debug` | Pico (RP2040) | Debug | `_build/pico-debug/`
| `pico_w-debug` | Pico W | Debug | `_build/pico_w-debug/`
| `pico_debug_probe-debug` | Pico Debug Probe | Debug | `_build/pico_debug_probe-debug/`
| `pico2-debug` | Pico 2 (RP2350) | Debug | `_build/pico2-debug/`
|===

Release presets share `build/` (one board at a time, same as the extension).
Debug presets use separate directories for parallel builds.

### Extra CMake Options

Pass additional `-D` flags when configuring:
[source,bash]
----
cmake --preset pico -DGIT_HASH=$GIT_HASH -DOPT_CDC_SYSVIEW=1 -DOPT_NET_SYSVIEW_SERVER=0
----

See the upstream <<how-to-build,Building from Source>> section and `CMakeLists.txt` for all `OPT_*` variables.

### Remotes

* `origin` -- this fork (`david-pedrosa/yapicoprobe`)
* `upstream` -- original repo (`rgrr/yapicoprobe`)
...
NOTE: This fork uses CMake Presets instead of the original Makefile.
See the <<Quick Start (this fork)>> section at the top for build instructions.

The original upstream build uses a Makefile wrapper around cmake.
Refer to the https://github.com/rgrr/yapicoprobe[upstream repository] for those instructions.

If you want to adopt YAPicoprobe options to your needs, check CMakeLists.txt and invoke cmake accordingly.

To see all available options:
`cmake -LH build/ | sed -n -e '/OPT_/{x;1!p;g;$!N;p;D;}'`
